...
(function () {
  ...
  function openChatPage(userObj, roomId) {
    ...
    var initial  = esc((userObj.username || userObj.name || 'U').charAt(0).toUpperCase());
    ...
    page.innerHTML =
      '<div id="chatPageHeader" style="display:flex;align-items:center;gap:12px;padding:12px 14px;background:rgba(0,8,28,0.95);border-bottom:1px solid rgba(0,100,255,0.2);flex-shrink:0;">'+
        ...
        '<div style="display:flex;gap:6px;">'+
          '<button style="background:none;border:none;color:rgba(255,255,255,0.6);cursor:pointer;padding:6px;">'+
      ...
      '<div style="display:flex;align-items:center;gap:8px;padding:10px 12px;background:rgba(0,6,20,0.98);border-top:1px solid rgba(0,80,200,0.2);flex-shrink:0;">'+
        ...
        '<div style="flex:1;display:flex;align-items:center;background:rgba(0,40,100,0.4);border:1.5px solid rgba(0,100,255,0.3);border-radius:24px;padding:0 14px;gap:6px;">'+
          ...
          '<button style="background:none;border:none;font-size:18px;cursor:pointer;padding:2px;">😊</button>'+
        '</div>'+
  ...
  function _renderMsgs(roomId) {
    ...
    (msgStore[roomId]||[]).forEach(function(msg) {
      ...
      row.innerHTML =
        '<div style="max-width:75%;min-width:60px;padding:8px 12px 6px;border-radius:'+(msg.isMe?'16px 16px 3px 16px':'16px 16px 16px 3px')+';background:'+(msg.isMe?'linear-gradient(135deg,#0065cc,#0045aa)':'rgba(255,255,255,0.07)')+';border:1px solid '+(msg.isMe?'rgba(0,120,255,0.25)':'rgba(255,255,255,0.07)')+';color:#fff;font-size:14px;line-height:1.45;word-break:break-word;">'+
          '<div>'+esc(msg.text)+'</div>'+
          '<div style="font-size:10px;color:rgba(255,255,255,0.38);margin-top:3px;text-align:right;">'+msg.time+(msg.isMe?'<span style="color:rgba(100,180,255,0.7);">✓✓</span>':'')+'</div>'+
        '</div>';
  ...
  function addOrFocusConversation(userObj, roomId) {
    ...
    if (existing) { dmList.insertBefore(existing, dmList.firstChild); openChatPage(userObj, roomId); return; }
    var item = document.createElement('div');
    item.className = 'conv-item';
    item.setAttribute('data-userid', userObj.userId);
    item.setAttribute('data-room', roomId);
    item.innerHTML =
      '<div class="avatar-wrap"><div class="avatar">'+esc((userObj.username||userObj.name||'U').charAt(0).toUpperCase())+'</div></div>'+
      '<div class="conv-info"><div class="conv-name">'+esc(userObj.username||userObj.name||'User')+'</div><div class="conv-preview">Tap to chat</div></div>'+
